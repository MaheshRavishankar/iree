// RUN: iree-opt --pass-pipeline="builtin.module(util.func(iree-dispatch-creation-fuse-horizontal-contractions, cse))" --split-input-file --mlir-print-local-scope %s | FileCheck %s
...
// CHECK-SAME:     %[[ARG1:[a-zA-Z0-9_]+]]: tensor<10x64x640xf16>
// CHECK-SAME:     %[[ARG2:[a-zA-Z0-9_]+]]: tensor<10x64x640xf16>
// CHECK-SAME:     %[[ARG3:[a-zA-Z0-9_]+]]: tensor<10x64x640xf16>
...
//  CHECK-DAG:       %[[EXTF_IN0:.+]] = arith.extf %[[IN0]]
//  CHECK-DAG:       %[[EXTF_IN1:.+]] = arith.extf %[[IN1]]
//  CHECK-DAG:       %[[MULF0:.+]] = arith.mulf %[[EXTF_IN0]], %[[EXTF_IN1]]
//  CHECK-DAG:       %[[ADDF0:.+]] = arith.addf %[[OUT0]], %[[MULF0]]
//  CHECK-DAG:       %[[EXTF_IN2:.+]] = arith.extf %[[IN2]]
//  CHECK-DAG:       %[[MULF1:.+]] = arith.mulf %[[EXTF_IN0]], %[[EXTF_IN2]]
//  CHECK-DAG:       %[[ADDF1:.+]] = arith.addf %[[OUT1]], %[[MULF1]]
//  CHECK-DAG:       %[[EXTF_IN3:.+]] = arith.extf %[[IN3]]
//  CHECK-DAG:       %[[MULF2:.+]] = arith.mulf %[[EXTF_IN0]], %[[EXTF_IN3]]
//  CHECK-DAG:       %[[ADDF2:.+]] = arith.addf %[[OUT2]], %[[MULF2]]
//  CHECK-DAG:       linalg.yield %[[ADDF0]], %[[ADDF1]], %[[ADDF2]]
//      CHECK:   %[[TRUNCF0:.+]] = linalg.generic
...
//      CHECK:   util.return %[[TRUNCF0]], %[[TRUNCF1]], %[[TRUNCF2]]
...
// CHECK-SAME:     %[[ARG0:[a-zA-Z0-9_]+]]: tensor<2x4096x640xi8>
// CHECK-SAME:     %[[ARG1:[a-zA-Z0-9_]+]]: tensor<640x640xi8>
// CHECK-SAME:     %[[ARG2:[a-zA-Z0-9_]+]]: tensor<640xi8>
// CHECK-SAME:     %[[ARG3:[a-zA-Z0-9_]+]]: tensor<f32>
// CHECK-SAME:     %[[ARG4:[a-zA-Z0-9_]+]]: tensor<640x640xi8>
// CHECK-SAME:     %[[ARG5:[a-zA-Z0-9_]+]]: tensor<640xi8>
// CHECK-SAME:     %[[ARG6:[a-zA-Z0-9_]+]]: tensor<f32>
...
//      CHECK:   util.return %[[TRUNC0]], %[[TRUNC1]]

// -----

util.func public @same_trunc_op(%arg0: tensor<2x4096x640xi8>, %arg1: tensor<2x640x640xi8>, %arg2: tensor<2x640x640xi8>) -> tensor<2x4096x640xf16> {
  %c0_i32 = arith.constant 0 : i32
  %0 = tensor.empty() : tensor<2x4096x640xf16>
  %1 = tensor.empty() : tensor<2x4096x640xi32>
  ...
  %4 = linalg.batch_matmul_transpose_b ins(%arg0, %arg2 : tensor<2x4096x640xi8>, tensor<2x640x640xi8>) outs(%2 : tensor<2x4096x640xi32>) -> tensor<2x4096x640xi32>
...
//      CHECK: util.func public @same_trunc_op
// CHECK-SAME:     %[[ARG0:.+]]: tensor<2x4096x640xi8>
// CHECK-SAME:     %[[ARG1:[a-zA-Z0-9_]+]]: tensor<2x640x640xi8>
// CHECK-SAME:     %[[ARG2:[a-zA-Z0-9_]+]]: tensor<2x640x640xi8>
//      CHECK:   %[[FUSED_OP:.+]]:2 = linalg.generic
