...
static std::optional<SmallVector<Operation *>> getHorizontalFusionGroupMembers(
  ...
  auto canBeGrouped = [&](linalg::LinalgOp linalgOp) -> bool {
    ...
    if (!isContractionOp(linalgOp, seedOp)) {
      return false;
    }
...
fuseGroup(RewriterBase &rewriter, ArrayRef<Operation *> fusionGroup,
          DominanceInfo &dominanceInfo) {
  if (!llvm::all_of(fusionGroup,
