...
func.func @matmul_ragged_lhs(%lhs: tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>,
                              %rhs: tensor<?x?xf32>,
                              %init: tensor<?x?xf32>) -> tensor<?x?xf32> {
  %result = linalg.matmul {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[64, 64, 0]]>}
      ins(%lhs, %rhs : tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>, tensor<?x?xf32>)
      outs(%init : tensor<?x?xf32>) -> tensor<?x?xf32>
  return %result : tensor<?x?xf32>
}
// CHECK-LABEL: func @matmul_ragged_lhs(
//       CHECK:   scf.forall
//       CHECK:   } {iree_codegen.sparse_iteration_dims = #iree_tensor_ext.sparse_iteration_dims<[0, 2]>, mapping = [#iree_codegen.workgroup_mapping<y>, #iree_codegen.workgroup_mapping<x>]}

// -----

// Test error case: sparse dimension accessed via non-AffineDimExpr.
func.func @matmul_ragged_non_affine_dim(%lhs: tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>,
                                         %rhs: tensor<?x?xf32>,
                                         %init: tensor<?x?xf32>) -> tensor<?x?xf32> {
...
func.func @matmul_multiple_ragged(%lhs: tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>,
                                   %rhs: tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>,
                                   %init: tensor<?x?xf32>) -> tensor<?x?xf32> {
  // expected-error @+1 {{unimplemented: same iteration dimension marked as sparse from different operands}}
  %result = linalg.matmul {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[64, 64, 0]]>}
      ins(%lhs, %rhs : tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>, tensor<?x?xf32, #iree_tensor_ext.ragged_tensor<0>>)
      outs(%init : tensor<?x?xf32>) -> tensor<?x?xf32>
  return %result : tensor<?x?xf32>
}
