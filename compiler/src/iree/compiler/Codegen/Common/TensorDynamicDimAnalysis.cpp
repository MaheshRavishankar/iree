...
// Transfer the range and divisibility information `source` value to `dest`
// value.
...
static void updateTensorDimInfo(
  ...
  for (auto [index, operand] : llvm::enumerate(dimOperands)) {
    updateTensorDimInfo(result, index, operand, solver, divisibilityInfo,
                        rangeInfo);
  }
}
